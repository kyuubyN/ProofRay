# Historical benchmark and research archive

> **Archive, not a current scorecard.** The sections below preserve the exact
> chronology of consumed-development runs, old engine snapshots, diagnostics,
> reopened holdouts and rejected hypotheses. A value here must not be read as a
> claim about the current ProofRay release unless the current
> [benchmark status page](../BENCHMARKS.md) explicitly promotes it with its
> protocol, metric and denominator.

> In particular, the historical LongMemEval value **0.7750** is the plain-arm
> comparator of one consumed-development paired judge run. It is neither the
> current engine's score nor an independent end-to-end accuracy result.

The maintained public entry point is [Benchmark status and claim boundaries](../BENCHMARKS.md).
