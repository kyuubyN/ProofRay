...
| MemGym consumer-endpoint (polish, local vs. cloud) | A consumed-development paired judge scored Horizon unpolished at 0.80, Horizon + cloud polish at 0.61, traditional BM25+cloud-LLM RAG (no Horizon) at 0.56, and Horizon + local 8-bit Qwen3-1.7B polish at 0.50 | A sealed, pre-registered, or product-advertised MemGym score, or a general small-model-amplification claim; the figures are consumed-development evidence on an inspected slice (arms A, B, D using public `DEFAULT_PROFILE` in `clean` mode, distinct from historical D144 `full_dossier`; arm C is an independent baseline; see `benchmarks/CONSUMED_DEVELOPMENT.md`) |
