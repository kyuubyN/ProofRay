...
"""ProofRay over MCP -- connect a chat client (Claude Desktop, Cursor, any MCP client)
directly to the deterministic answer engine, exposing `proofray_ask` and the
compatibility alias `horizon_ask`.
...
decides for itself, from its own read of the conversation, when calling `proofray_ask` is relevant
...
def _horizon_ask_impl(question: str, documents: list[str | dict], include_sources: bool = False,
    ...
    """The plain-Python implementation shared by both MCP tool names -- kept separate from the
...
mcp = MCPServer("proofray")


@mcp.tool()
async def proofray_ask(question: str, documents: list[str | dict], include_sources: bool = False,
                       polish: bool = False, polish_model: str | None = None,
                       context_intents: list[dict] | None = None) -> dict:
    """Ask ProofRay over caller-supplied documents and return verified evidence or abstention."""
...
async def horizon_ask(question: str, documents: list[str | dict], include_sources: bool = False,
                      polish: bool = False, polish_model: str | None = None,
                      context_intents: list[dict] | None = None) -> dict:
    """Compatibility alias for :func:`proofray_ask` over a caller-supplied document set.
    Call it when the conversation genuinely calls for recalling something previously established -- deciding
    ...
    server's own deployment config (PROOFRAY_POLISH_BASE_URL / PROOFRAY_POLISH_API_KEY_ENV), not
